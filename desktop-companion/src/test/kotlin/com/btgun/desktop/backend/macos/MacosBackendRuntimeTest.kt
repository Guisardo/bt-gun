...
fun main() {
    runtimePublishesTrustedUdpCallbackThroughMacosBackend()
    ...
    diagnosticsCallbackReportsMetadataOnly()
    runtimeDoesNotOwnUdpSocketsPairingOrAuth()
}

private fun runtimePublishesTrustedUdpCallbackThroughMacosBackend() {
    val helper = RuntimeFakeMacosHidHelper()
    val backend = MacosVirtualControllerBackend(helper = helper)
    val runtime = MacosBackendRuntime(
        config = MacosBackendRuntimeConfig(helperPath = "/tmp/BtGunMacosHidHelper"),
        backend = backend,
    ...
    val expected = MacosHidReportPacker.packInputReport(UdpControllerStateAdapter.toState(input))
    expectEquals("backend started", BackendLifecycleState.STARTED, backend.lifecycleState)
    expectEquals("one helper submission", 1, helper.submitted.size)
    expectByteArrayEquals("submitted packed report", expected.bytes, helper.submitted.single().bytes)
    expectEquals("report metadata sequence", 42L, helper.submitted.single().sourceSequence)
    expectTrue("last publish recorded", runtime.diagnostics().lastPublishResult is BackendPublishResult.Published)
}
...
private fun runtimePreservesExistingUdpCallbackWhenAttached() {
    ...
    val runtime = MacosBackendRuntime(
        config = MacosBackendRuntimeConfig(helperPath = "/tmp/BtGunMacosHidHelper"),
        backend = MacosVirtualControllerBackend(helper = helper),
        nowElapsedNanos = { 3_000_000_000L },
    )
    val server = ControlServer(registry = testRegistry())
    ...
    expectEquals("runtime still published", 1, helper.submitted.size)
}
...
private fun staleInputPublishClearsButtonsAndStickButKeepsAim() {
    ...
    val bytes = helper.submitted.single().bytes
...
private fun backendOutputReportRoutesToAuthenticatedPhoneHaptic() {
    ...
    expectEquals("command id from output report drain", "macos-output-report-1", envelope?.body?.stringField("commandId"))
...
private fun diagnosticsCallbackReportsMetadataOnly() {
    val helper = RuntimeFakeMacosHidHelper()
    helper.status = MacosHidHelperStatus(deviceActive = true, inputReportsSubmitted = 1L)
    val runtime = MacosBackendRuntime(
        config = MacosBackendRuntimeConfig(helperPath = "/tmp/BtGunMacosHidHelper"),
        backend = MacosVirtualControllerBackend(helper = helper),
        nowElapsedNanos = { 3_000_000_000L },
    ...
    val diagnosticsRecorder: (Any) -> Unit = { backendDiagnostics -> observed.add(backendDiagnostics.toString()) }
    runtime.onDiagnosticsChanged = diagnosticsRecorder
    val server = ControlServer(registry = testRegistry())

    runtime.attach(server)
    server.onUdpInputReceived(receivedInput(stale = true, sequence = 47L))

    expectTrue("diagnostics callback fired", observed.isNotEmpty())
    val latest = runtime.diagnostics()
    expectEquals("lifecycle", BackendLifecycleState.STARTED, latest.lifecycleState)
    expectEquals("publish", BackendPublishResult.Published, latest.lastPublishResult)
    expectEquals("stale", true, latest.stale)
    expectEquals("source sequence", 47L, latest.lastSourceSequence)
    expectEquals("haptic result", null, latest.lastHapticSendResult)
    expectEquals("haptic routed count", 0L, latest.outputHapticCommandsRouted)
    expectEquals("helper status", helper.status, latest.helperStatus)
    expectNoForbiddenDiagnosticMaterial(latest.toString())
}

private fun runtimeDoesNotOwnUdpSocketsPairingOrAuth() {
    val source = Files.readString(Paths.get("src/main/kotlin/com/btgun/desktop/backend/macos/MacosBackendRuntime.kt"))
    listOf(
        "import com.btgun.desktop.control.ControlServer",
        "import com.btgun.desktop.transport.UdpReceivedInput",
        "import com.btgun.desktop.backend.UdpControllerStateAdapter",
    ).forEach { expected ->
        expectContains("required import", source, expected)
    }
    listOf(
        "io.ktor",
        "DatagramSocket",
        ...
        expectFalse("runtime excludes $forbidden", source.contains(forbidden, ignoreCase = true))
    }
}

private class RuntimeFakeMacosHidHelper : MacosHidHelperClient {
    ...
    override fun submitInputReport(report: MacosInputReport): MacosHidHelperResult {
        ...
        return MacosHidHelperResult.Ok
    }
...
private fun expectNoForbiddenDiagnosticMaterial(text: String) {
    listOf(
        "raw packet",
        "QR",
        "proof",
        ...
        expectFalse("diagnostics exclude $forbidden", text.contains(forbidden, ignoreCase = true))
    }
}
