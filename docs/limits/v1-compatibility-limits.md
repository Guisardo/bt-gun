...
|------|--------|-------------|----------|-------------------|
| Android phone as Bluetooth HID gamepad for macOS input | supported | macOS Apple Silicon sees the Android phone as a normal gamepad-style joystick for gun controls and aim in OS, Game Controller, and browser Gamepad API surfaces. | `docs/evidence/manifests/phase7-android-bluetooth-hid.jsonl#phase7-gamecontroller-input` | Rerun Phase 7 proof after descriptor or SDP changes. |
...
| Steam app detection through Android Bluetooth HID | deferred | Browser Gamepad API visibility does not prove Steam app detection. The debug host app now has a diagnostic-only boring-standard HID profile for Steam/SDL retesting; the user app stays on the user-facing profile. | `docs/evidence/manifests/phase7-android-bluetooth-hid.jsonl#phase7-steam-app-detection` | Install rebuilt `android-host/app`, forget/re-pair the Bluetooth HID gamepad, then prove Steam Controller Settings detects the diagnostic profile. |
