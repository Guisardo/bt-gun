...
| Android motion aim capture | Original experience depends on phone-based aiming | MEDIUM | Use SensorManager; prefer fused rotation/game-rotation-vector or gyro+accelerometer data, and support accelerometer/gravity tilt fallback when no gyroscope exists. |
...
| Reliable control/haptic channel | Phone haptics are required in v1 and need acknowledgements/failure states | MEDIUM | Use TCP/WebSocket control channel separate from UDP input. |
...
| Joystick visualizer | First validation target must show full end-to-end behavior | MEDIUM | Should show buttons, sticks, mapped aim axes, latency, recenter state, and haptic test. |
...
| Bidirectional haptics with TTL | Makes feedback visible and avoids stuck vibration | MEDIUM | Include command ids, duration, strength, pattern, ack/failure. |
| Profile-based motion mapping | Supports multiple games/tools without Android rebuilds | MEDIUM | Desktop owns mapping; Android sends raw/normalized motion with provider/capability metadata. |
...
|---------|---------------|-----------------|-------------|
| Direct desktop Bluetooth to gun | Removes Android phone from chain | Loses phone motion-sensor role and multiplies platform-specific Bluetooth work | Android host app for v1 |
...
- **Haptics require bidirectional control channel:** UDP input alone is not enough for acknowledged haptic commands.
...
- [ ] Android captures motion aim data with timestamps, including fused rotation, gyro, accelerometer, gravity, or tilt fallback as available.
- [ ] Reload-hold for two seconds recenters aim.
- [ ] Desktop and Android pair by QR or code over LAN.
- [ ] Android streams normalized input/motion frames under a 50 ms visualizer target.
...
- [ ] Desktop sends haptic command back to Android and the phone vibrates.
- [ ] Joystick visualizer proves buttons, axes, recenter, latency, and phone haptics.
...
- [ ] Physical gun motor rumble - defer until phone-haptic v1 works and a motor command path is proven.
...
| Android motion aim capture/recenter | HIGH | MEDIUM | P1 |
...
| Phone haptic round-trip | HIGH | MEDIUM | P1 |
...
| Phone motion aim | Original AR design uses Android motion | Desktop virtual pads do not solve phone motion sensors | Android captures motion samples and desktop maps profiles. |
...
| Haptics | Phone vibration confirmed; hardware motor deferred | Varies by backend/API | Make phone haptic round-trip a v1 acceptance check. |
