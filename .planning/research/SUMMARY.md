...
Bluetooth Gun Driver should be built as a three-part system: an Android gun server app, a local LAN transport, and native desktop virtual gamepad backends. Android owns the physical iPega gun and phone gyro because that matches the discontinued product's original model. Desktop owns profile mapping and virtual HID exposure because Windows/macOS behavior and game expectations belong on the host platform.

The recommended v1 proof is not a real game. It is a simple joystick visualizer that can show every control, mapped aim axes, recenter behavior, packet timing, and rumble round-trip. This avoids mixing driver/protocol risk with game-specific quirks.

The main technical risks are reverse-engineering the iPega Bluetooth protocol, shipping product-grade virtual HID paths on both desktop OSes, and keeping aim latency under 50 ms. Research suggests starting with hardware/protocol discovery, then a timestamped UDP input protocol plus reliable control/rumble channel, then desktop HID backend spikes, then the visualizer/profile layer.

## Key Findings

### Recommended Stack

Use native Android/Kotlin for the host app, Android Bluetooth and SensorManager APIs for gun/gyro capture, UDP for high-rate input frames, TCP/WebSocket for pairing/control/rumble, Windows VHF/KMDF for product-grade virtual HID, and macOS CoreHID or HIDDriverKit depending on OS/entitlement feasibility.

**Core technologies:**
- Android Bluetooth APIs: physical iPega connection and protocol bridge.
- Android SensorManager: gyro/rotation-vector capture with monotonic timestamps.
- UDP input protocol: low-latency aim/input stream.
- TCP/WebSocket control channel: pairing, heartbeat, diagnostics, rumble.
...
- Android captures gyro/rotation data and supports reload-hold recenter.
- Desktop pairs with Android by QR or code over local LAN.
- Android streams normalized input/gyro to desktop under the 50 ms visualizer target.
- Windows and macOS expose a regular virtual gamepad/joystick gun.
- Desktop profiles map aim to configurable joystick axes.
- Desktop sends rumble back to the physical gun.
...
- Visible latency, packet loss, connection, and rumble diagnostics.
...
**Major components:**
1. Android gun adapter - connects to and decodes iPega gun input/rumble.
2. Android sensor capture - reads gyro/rotation samples and handles recenter.
3. LAN session transport - QR/code pairing, encrypted UDP input, reliable control/rumble.
...
5. **Rumble without TTL/ack** - include command ids, duration, expiry, and ack/fail.
...
**Addresses:** Gun connection, input decode, rumble capability discovery.
...
**Delivers:** QR/code pairing, encrypted session, UDP InputFrame, reliable control/rumble channel, latency instrumentation.
...
**Delivers:** Fake-input virtual gamepad on Windows and macOS, rumble output callback/loopback, backend capability interface.
...
**Delivers:** Android host streams real gun/gyro input to desktop visualizer; desktop profiles map aim axes; reload-hold recenters; rumble round-trip works.
...
- **Phase 3:** Windows rumble semantics depend on HID descriptor/output report and target input APIs.
...
- **Rumble semantics:** Capture original app/hardware behavior and design desktop output report mapping accordingly.
