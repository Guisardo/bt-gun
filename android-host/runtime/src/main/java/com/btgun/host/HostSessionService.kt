...
import com.btgun.host.motion.AimCalibrationCodec
...
import com.btgun.host.haptics.HapticTimelinePulse
...
import com.btgun.host.play.PlayMode
import com.btgun.host.play.PlayModeController
...
internal fun shouldStartInputSessionForPlayMode(state: HostSessionState): Boolean =
...
private const val RECENTER_CONTROL_STUCK_RELEASE_NANOS: Long = 12_000_000_000L
...
class HostSessionService : Service() {
    ...
    private val playModeController = PlayModeController()
    private var softControlSequence: Long = 1L
    ...
    private val gunConnectionWatchdogTick = Runnable {
        handleGunConnectionWatchdog()
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when (intent?.action) {
            ...
            ACTION_RUN_LAN_HAPTIC_TEST -> runLanHapticTest()
    ...
    private fun startSession() {
        ...
        handler.removeCallbacks(gunConnectionWatchdogTick)
        ...
        val listener = object : IpegaBleGunAdapter.Listener {
            override fun onConnectionState(state: BleGunConnectionState) {
                ...
                when (state.phase) {
                    BleGunConnectionPhase.CONNECTED,
                    BleGunConnectionPhase.ERROR,
                    BleGunConnectionPhase.STOPPED,
                    -> handler.removeCallbacks(gunConnectionWatchdogTick)
                    else -> Unit
                }
                if (shouldClearGunInput) {
        ...
        handler.postDelayed(gunConnectionWatchdogTick, GUN_CONNECTION_WATCHDOG_MILLIS)
    }
    ...
    private fun handleGunConnectionWatchdog() {
        if (currentState.phase !in setOf(
                HostSessionPhase.SCANNING,
                HostSessionPhase.CONNECTING,
                HostSessionPhase.RECONNECTING,
                HostSessionPhase.STARTING,
        ...
        currentState = currentState.copy(
            ...
            lastBleConnectionState = BleGunConnectionState(
    ...
    private fun startBluetoothGamepad(openPairingWindow: Boolean = false) {
        ...
        if (!canStartWithPermissionGate(gate)) {
            ...
            AndroidLog.w(TAG, "startBluetoothGamepad input blocked: ${currentState.lastError}")
            return
        }
        ...
        if (shouldStartInputSessionForPlayMode(currentState)) {
            startSession()
        }
        if (currentState.phase == HostSessionPhase.ERROR) {
            AndroidLog.w(TAG, "startBluetoothGamepad blocked after input start: ${currentState.lastError}")
            return
        }
        ...
        if (playModeController.mode != PlayMode.BLUETOOTH_HID) {
            stopUdpInput()
            playModeController.switchTo(PlayMode.BLUETOOTH_HID)
        }
    ...
    private fun reloadActiveProfile() {
        ...
        loadCalibrationForActiveProfile()
    ...
    private fun startUdpInput(config: InputStreamConfig) {
        ...
        if (playModeController.mode != PlayMode.LAN) {
            currentState = hidSessionController.stopBluetoothGamepad(currentState)
            playModeController.switchTo(PlayMode.LAN)
        }
        stopUdpInput()
        playModeController.switchTo(PlayMode.LAN)
    ...
    private fun sendUdpSnapshot(throttled: Boolean = false) {
        if (!playModeController.canSendLan) {
            return
        }
    ...
    private fun scheduleReloadHoldTick() {
        ...
        if (!recenter.state.isReloadHeld || pressedAt == null || recenter.state.recenterEmitted) {
            return
        }
        val targetNanos = pressedAt + ReloadHoldRecenter.RELOAD_HOLD_NANOS
    ...
    private fun startAimCalibrationFromMenu() {
        startAimCalibration()
        currentState = currentState.copy(aimCalibrationState = aimCalibrationSession.state)
        publishVisualizerStatus()
    }

    private fun handleSoftControl(control: String?, pressed: Boolean) {
        val controlId = control?.takeIf { it in SOFT_CONTROL_IDS } ?: return
        val now = SystemClock.elapsedRealtimeNanos()
        val envelope = LiveEnvelope(
            stream = StreamKind.GUN,
            seq = softControlSequence++,
            captureElapsedNanos = now,
            emittedElapsedNanos = now,
            payload = GunEvent(name = controlId, pressed = pressed),
        )
        val nextGunInput = currentState.gunInputState.apply(envelope.payload)
        currentState = profileRuntime.mapCurrentState(currentState.copy(
            lastGunEvent = envelope,
            gunInputState = nextGunInput,
        ))
        fanOutBluetoothHidLiveInput()
        sendUdpEdge(envelope)
    }

    private fun runLanHapticTest() {
        val now = SystemClock.elapsedRealtimeNanos()
        if (!playModeController.canSendLan || currentState.packetStreamState != InputStreamLifecycleState.ACTIVE) {
            currentState = currentState.copy(
                desktopLinkState = currentState.desktopLinkState.copy(
                    lastControlError = "LAN haptic test requires active LAN mode.",
                ),
            )
            return
        }
        val result = desktopHapticExecutor.handle(
            DesktopHapticCommand(
                commandId = "lan-haptic-test-$now",
                strength = 0.7,
                durationMs = 90L,
                ttlMs = 1_000L,
                pattern = "menu-test",
                patternTimeline = listOf(
                    HapticTimelinePulse(atMs = 0L, durationMs = 80L, strength = 0.45),
                    HapticTimelinePulse(atMs = 140L, durationMs = 110L, strength = 0.85),
                    HapticTimelinePulse(atMs = 330L, durationMs = 70L, strength = 0.55),
                ),
            ),
            receivedElapsedNanos = now,
        )
        desktopControlClient?.sendHapticResult(result)
        currentState = currentState.copy(
            desktopLinkState = currentState.desktopLinkState.copy(
                lastControlError = "LAN haptic test ${result.status.wireName}: ${result.detail}",
            ),
        )
    }
    ...
    private fun fanOutBluetoothHidLiveInput() {
        if (playModeController.canSendBluetoothHid) {
    ...
    companion object {
        ...
        const val ACTION_RUN_LAN_HAPTIC_TEST: String = "com.btgun.host.action.RUN_LAN_HAPTIC_TEST"
        ...
        const val EXTRA_SOFT_CONTROL: String = "com.btgun.host.extra.SOFT_CONTROL"
        ...
        private const val GUN_CONNECTION_WATCHDOG_MILLIS: Long = 20_000L
        ...
        private val SOFT_CONTROL_IDS = setOf("back", "home", "select")
