...
class ProfileMapper(
    ...
    fun map(
        ...
        softPressedControls: Set<SoftControl> = emptySet(),
        ...
        if (selected.source == AIM_SOURCE_CENTER) {
            ...
            return MappedControllerState(
                ...
                pressedVirtualControls = profile.mapPressedVirtualControls(gunInputState, softPressedControls),
    ...
    private fun BtGunProfile.mapPressedVirtualControls(
        gunInputState: GunInputState,
        softPressedControls: Set<SoftControl>,
    ): Set<String> =
        buildSet {
            gunInputState.pressedControls.mapNotNullTo(this) { controlId ->
                val physical = PhysicalButton.fromId(controlId)
                val virtual = if (physical != null) {
                    buttonMapping[physical]
                } else {
                    SoftControl.fromId(controlId)?.let { soft -> softControlMapping[soft] }
                }
                virtual?.id
            }
            softPressedControls.mapNotNullTo(this) { control -> softControlMapping[control]?.id }
        }
