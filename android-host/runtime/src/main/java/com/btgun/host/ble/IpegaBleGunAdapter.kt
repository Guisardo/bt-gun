...
class IpegaBleGunAdapter(
    ...
    private val scanCallback = object : ScanCallback() {
        override fun onScanResult(callbackType: Int, result: ScanResult) {
            ...
            emitStatus("ble_scan", "candidate_found:${name ?: "unnamed"}:${serviceUuids.joinToString(",")}")
            stopScan()
    ...
    private val gattCallback = object : BluetoothGattCallback() {
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
            when (newState) {
                BluetoothProfile.STATE_CONNECTED -> {
                    ...
                    scheduleConnectionTimeout("gatt service discovery", GATT_DISCOVERY_TIMEOUT_MILLIS)
                    try {
        ...
        override fun onServicesDiscovered(gatt: BluetoothGatt, status: Int) {
            ...
            scheduleConnectionTimeout("fff3 notification setup", NOTIFICATION_SETUP_TIMEOUT_MILLIS)
            subscribeFff3(gatt)
        }
    ...
    private fun connectGatt(device: BluetoothDevice) {
        ...
        scheduleConnectionTimeout("gatt connect", GATT_CONNECT_TIMEOUT_MILLIS)
        try {
    ...
    private fun scheduleScanTimeout() {
        ...
        val timeout = Runnable {
            if (active && candidateDevice == null) {
                handleError("ble scan timeout after ${BLE_SCAN_TIMEOUT_MILLIS}ms; target $ARGUN_DEVICE_NAME/$BLE_SERVICE_UUID not found")
            }
        }
        scanTimeout = timeout
        handler.postDelayed(timeout, BLE_SCAN_TIMEOUT_MILLIS)
    }
    ...
    private fun scheduleConnectionTimeout(stage: String, timeoutMillis: Long) {
        cancelConnectionTimeout()
        val timeout = Runnable {
            if (active && activeGatt != null) {
                handleError("$stage timeout after ${timeoutMillis}ms")
            }
        }
    ...
    companion object {
        ...
        const val BLE_SCAN_TIMEOUT_MILLIS: Long = 12_000L
        const val GATT_CONNECT_TIMEOUT_MILLIS: Long = 8_000L
        const val GATT_DISCOVERY_TIMEOUT_MILLIS: Long = 8_000L
        const val NOTIFICATION_SETUP_TIMEOUT_MILLIS: Long = 8_000L
        ...
        fun matchesTargetAdvertisement(deviceName: String?, advertisedServiceUuids: Set<String>): Boolean {
            val serviceMatch = advertisedServiceUuids.any { uuid -> uuid.equals(BLE_SERVICE_UUID, ignoreCase = true) }
