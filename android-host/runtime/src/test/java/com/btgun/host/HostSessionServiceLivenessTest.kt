...
private fun playModeStartsSharedInputSessionWhenIdle() {
    expectTrue("idle needs input", shouldStartInputSessionForPlayMode(HostSessionState()))
    expectTrue("stopped needs input", shouldStartInputSessionForPlayMode(HostSessionState(phase = HostSessionPhase.STOPPED)))
    expectTrue("error needs input", shouldStartInputSessionForPlayMode(HostSessionState(phase = HostSessionPhase.ERROR)))
    expectFalse("scanning already input", shouldStartInputSessionForPlayMode(HostSessionState(phase = HostSessionPhase.SCANNING)))
    expectFalse("connected already input", shouldStartInputSessionForPlayMode(HostSessionState(phase = HostSessionPhase.CONNECTED)))
    expectFalse("reconnecting already input", shouldStartInputSessionForPlayMode(HostSessionState(phase = HostSessionPhase.RECONNECTING)))
}
